using RobotPointsRenumber.Model.Operations.ABB;
using System;
...
namespace RobotPointsRenumber
{

    public class RenameWindowViewModel : ViewModelBase, INotifyPropertyChanged
    {
        private List<string> modifiedList;


        public RenameWindowViewModel(Dictionary<string, ABBModule> foundModules)
        {
                ...
                case 1:
                    {
                        ...
                        foreach (var point in FoundPoints)
                        {
                            ...
                            {
                                string newName = Prefix + ((startNum + enumerator * counter).ToString());
                                tempList.Add(newName);

                                FoundModules[SelectedModule].ProceduresList.First(x => x.Name.ToLower() == SelectedProcedure.ToLower()).SetNewName(newName, counter2);
                                counter++;
                            }
                            else
                            {
        ...
        private void Apply()
        {
            ...
            FoundModules[SelectedModule].Modified = true;
            SelectedPointsList = new List<string>();
        }
        ...
        {
            if (string.IsNullOrEmpty(Prefix) || string.IsNullOrEmpty(Enumerator) || string.IsNullOrEmpty(StartNum) || int.Parse(Enumerator) < 1 || int.Parse(StartNum) < 1 || SelectedProcedure == null)
                ApplyEnabled = false;
            else
                ApplyEnabled = true;
        }
        ...
        private void CheckSinglePointInputEnabled()
        {
            ...
            else
            {
                ...
                foreach (var item in SelectedPointsList)
                {
                    if (founditems.Count == 0 || founditems.Contains(item))
                        founditems.Add(item);
                    else
                    {
                        SinglePointInputEnabled = false;
